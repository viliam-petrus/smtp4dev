...
namespace Rnwood.Smtp4dev.Server
{
    internal static class CertificateHelper
    {
        ...
        public static X509Certificate2 GetTlsCertificateForPop3(ServerOptions options, ILogger logger)
        {
            X509Certificate2 cert = null;


            // Only consider Pop3TlsMode when deciding whether a certificate is required for POP3
            if (options.Pop3TlsMode != TlsMode.None)
            {
                if (!string.IsNullOrEmpty(options.TlsCertificateStoreThumbprint))
                {
                    cert = CertificateHelper.LoadCertificateWithThumbprint(options.TlsCertificateStoreThumbprint);
